...
import { cssVariables } from '@/shared/lib/cssVariables'
...
export const ImageMedia: React.FC<MediaProps> = (props) => {
  ...
  // and back even though nothing actually needs to load. A preloaded LCP
  // image can finish between the first `complete` check and React attaching
  // its synthetic `onLoad`, so retain a native one-shot listener as a bridge.
  useLayoutEffect(() => {
  ...
  const shouldRevealImmediately = disableFadeIn || priority
  ...
  return (
    <picture
      className={cn(
        ...
        shouldRevealImmediately || isLoaded ? 'opacity-100' : 'opacity-0',
        !shouldRevealImmediately &&
          !skipTransition &&
          reveal === 'soft' &&
          (isLoaded
